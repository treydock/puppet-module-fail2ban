...
describe 'Puppet::Type.type(:fail2ban_jail_config)' do
  let(:fail2ban_jail_config) do
    Puppet::Type.type(:fail2ban_jail_config).new(name: 'vars/foo', value: 'bar')
  end
  ...
  it 'does not expect a name with whitespace' do
    ...
    }.to raise_error(Puppet::Error, %r{Invalid fail2ban_jail_config})
  end
  ...
  describe 'autorequire File resources' do
    it 'autorequires /etc/fail2ban/jail.local' do
